...
def parse_extrinsics(file_path):
    ...
    is_orthonormal, is_valid_determinant = check_rotation_matrix(W2C[:3, :3])
    print("is_orthonormal: ", is_orthonormal)
    print("is_valid_determinant: ", is_valid_determinant)
...
def parse_intrinsics(file_path):
    ...
    w, h = intrinsics.get(0, 0), intrinsics.get(1, 0)
...
def process_cameras(metadata_dir, cameras_dir, flip_R, flip_T):
    ...
    for extr_file in extrinsics_files:
        ...
        try:
            ...
            extrinsic = parse_extrinsics(extr_path)
...
def process_images(color_dir, rectified_dir):
    ...
    frame_ids = [34, 35, 36]
    color_files = [f for f in os.listdir(color_dir) if f.lower().endswith(supported_formats) and int(f.split('_')[0]) in frame_ids]
